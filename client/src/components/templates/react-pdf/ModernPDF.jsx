import React from "react";
import {
  Document,
  Page,
  Text,
  View,
  StyleSheet,
  Font,
  Svg,
  Path,
} from "@react-pdf/renderer";

// Register fonts
try {
  Font.register({
    family: "Open Sans",
    fonts: [
      {
        src: "https://cdn.jsdelivr.net/npm/open-sans-all@0.1.3/fonts/open-sans-regular.ttf",
      },
      {
        src: "https://cdn.jsdelivr.net/npm/open-sans-all@0.1.3/fonts/open-sans-600.ttf",
        fontWeight: 600,
      },
      {
        src: "https://cdn.jsdelivr.net/npm/open-sans-all@0.1.3/fonts/open-sans-700.ttf",
        fontWeight: 700,
      },
    ],
  });
} catch (error) {
  console.error("Error registering fonts:", error);
}

// Create styles
const styles = StyleSheet.create({
  page: {
    fontFamily: "Open Sans",
    backgroundColor: "white",
    // padding: 12,
  },
  header: {
    padding: "8 12",
    backgroundColor: "#F9FAFB",
    marginBottom: 8,
  },
  headerName: {
    fontSize: 16,
    fontWeight: 700,
    color: "#111827",
    marginBottom: 4,
  },
  contactInfo: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 4,
  },
  contactItem: {
    flexDirection: "row",
    alignItems: "center",
    fontSize: 8,
    color: "#4B5563",
    marginRight: 8,
  },
  content: {
    padding: "0 12",
  },
  section: {
    marginBottom: 8,
  },
  sectionTitle: {
    fontSize: 11,
    fontWeight: 700,
    color: "#111827",
    borderBottomWidth: 1,
    borderBottomColor: "#E5E7EB",
    paddingBottom: 2,
    marginBottom: 6,
  },
  summary: {
    fontSize: 9,
    color: "#374151",
    lineHeight: 1.4,
  },
  skillsContainer: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 4,
  },
  skillCategory: {
    marginBottom: 4,
  },
  skillCategoryTitle: {
    fontSize: 9,
    fontWeight: 600,
    color: "#111827",
  },
  skillCategoryItems: {
    fontSize: 9,
    color: "#374151",
  },
  experienceItem: {
    marginBottom: 8,
  },
  jobHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "flex-start",
    marginBottom: 4,
  },
  jobTitle: {
    fontSize: 10,
    fontWeight: 600,
    color: "#111827",
  },
  jobCompany: {
    fontSize: 9,
    color: "#4B5563",
  },
  dates: {
    fontSize: 9,
    color: "#6B7280",
  },
  bulletList: {
    paddingLeft: 4,
  },
  bulletItem: {
    fontSize: 9,
    color: "#374151",
    marginBottom: 2,
    lineHeight: 1.4,
    flexDirection: "row",
  },
  bullet: {
    marginRight: 4,
  },
  bulletText: {
    fontSize: 9,
    flex: 1,
  },
  educationItem: {
    marginBottom: 6,
  },
  gpa: {
    fontSize: 9,
    color: "#4B5563",
    marginTop: 2,
  },
  awardItem: {
    marginBottom: 6,
  },
  awarderText: {
    fontSize: 9,
    color: "#4B5563",
  },
  awardSummary: {
    fontSize: 9,
    color: "#374151",
    marginTop: 2,
  },
  footer: {
    fontSize: 8,
    color: "#9ca3af",
    textAlign: "center",
    marginTop: 16,
  },
  icon: {
    width: 10,
    height: 10,
    marginRight: 4,
  },
  contactItemContent: {
    flexDirection: "row",
    alignItems: "center",
  }
});

// Icon components
const IconComponent = ({ children }) => (
  <Svg style={styles.icon} viewBox="0 0 512 512" width="10" height="10">
    {children}
  </Svg>
);

const Icons = {
  Email: () => (
    <IconComponent>
      <Path
        d="M48 64C21.5 64 0 85.5 0 112c0 15.1 7.1 29.3 19.2 38.4L236.8 313.6c11.4 8.5 27 8.5 38.4 0L492.8 150.4c12.1-9.1 19.2-23.3 19.2-38.4c0-26.5-21.5-48-48-48H48zM0 176V384c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V176L294.4 339.2c-22.8 17.1-54 17.1-76.8 0L0 176z"
        fill="#6B7280"
      />
    </IconComponent>
  ),
  Phone: () => (
    <IconComponent>
      <Path
        d="M164.9 24.6c-7.7-18.6-28-28.5-47.4-23.2l-88 24C12.1 30.2 0 46 0 64C0 311.4 200.6 512 448 512c18 0 33.8-12.1 38.6-29.5l24-88c5.3-19.4-4.6-39.7-23.2-47.4l-96-40c-16.3-6.8-35.2-2.1-46.3 11.6L304.7 368C234.3 334.7 177.3 277.7 144 207.3L193.3 167c13.7-11.2 18.4-30 11.6-46.3l-40-96z"
        fill="#6B7280"
      />
    </IconComponent>
  ),
  Location: () => (
    <IconComponent>
      <Path
        d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z"
        fill="#6B7280"
      />
    </IconComponent>
  ),
  Linkedin: () => (
    <IconComponent>
      <Path
        d="M416 32H31.9C14.3 32 0 46.5 0 64.3v383.4C0 465.5 14.3 480 31.9 480H416c17.6 0 32-14.5 32-32.3V64.3c0-17.8-14.4-32.3-32-32.3zM135.4 416H69V202.2h66.5V416zm-33.2-243c-21.3 0-38.5-17.3-38.5-38.5S80.9 96 102.2 96c21.2 0 38.5 17.3 38.5 38.5 0 21.3-17.2 38.5-38.5 38.5zm282.1 243h-66.4V312c0-24.8-.5-56.7-34.5-56.7-34.6 0-39.9 27-39.9 54.9V416h-66.4V202.2h63.7v29.2h.9c8.9-16.8 30.6-34.5 62.9-34.5 67.2 0 79.7 44.3 79.7 101.9V416z"
        fill="#6B7280"
      />
    </IconComponent>
  ),
  Github: () => (
    <IconComponent>
      <Path
        d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z"
        fill="#6B7280"
      />
    </IconComponent>
  ),
};

// Modern PDF Component
const ModernPDF = ({ resume }) => {
  const {
    basics = {},
    experience = [],
    education = [],
    skills = {
      programmingLanguages: [],
      librariesFrameworks: [],
      databases: [],
      toolsPlatforms: [],
      apis: [],
    },
    projects = [],
    awards = [],
  } = resume || {};

  // Check if an array has content
  const hasContent = (arr) => Array.isArray(arr) && arr.length > 0;

  // Check if skills has any non-empty arrays
  const hasSkills = Object.values(skills).some(
    skillArray => hasContent(skillArray)
  );

  try {
    return (
      <Document>
        <Page size="A4" style={styles.page}>
          {/* Header */}
          <View style={styles.header}>
            <Text style={styles.headerName}>{basics?.name || "Your Name"}</Text>
            <View style={styles.contactInfo}>
              {basics?.email && (
                <View style={styles.contactItemContent}>
                  <Icons.Email />
                  <Text style={styles.contactItem}>{basics.email}</Text>
                </View>
              )}
              {basics?.phone && (
                <View style={styles.contactItemContent}>
                  <Icons.Phone />
                  <Text style={styles.contactItem}>{basics.phone}</Text>
                </View>
              )}
              {basics?.location && (
                <View style={styles.contactItemContent}>
                  <Icons.Location />
                  <Text style={styles.contactItem}>{basics.location}</Text>
                </View>
              )}
              {basics?.linkedin && (
                <View style={styles.contactItemContent}>
                  <Icons.Linkedin />
                  <Text style={styles.contactItem}>{basics.linkedin}</Text>
                </View>
              )}
              {basics?.github && (
                <View style={styles.contactItemContent}>
                  <Icons.Github />
                  <Text style={styles.contactItem}>{basics.github}</Text>
                </View>
              )}
            </View>
          </View>

          <View style={styles.content}>
            {/* Summary */}
            {basics?.summary && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Summary</Text>
                <Text style={styles.summary}>{basics.summary}</Text>
              </View>
            )}

            {/* Skills */}
            {hasSkills && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Technical Skills</Text>
                {Object.entries({
                  "Programming Languages": skills?.programmingLanguages || [],
                  "Libraries & Frameworks": skills?.librariesFrameworks || [],
                  "Databases": skills?.databases || [],
                  "Tools & Platforms": skills?.toolsPlatforms || [],
                  "APIs": skills?.apis || [],
                }).map(
                  ([category, skillList]) =>
                    hasContent(skillList) && (
                      <View key={category} style={styles.skillCategory}>
                        <Text>
                          <Text style={styles.skillCategoryTitle}>{category}: </Text>
                          <Text style={styles.skillCategoryItems}>{skillList.join(", ")}</Text>
                        </Text>
                      </View>
                    )
                )}
              </View>
            )}

            {/* Experience */}
            {hasContent(experience) && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Experience</Text>
                {experience.map((job, index) => (
                  <View key={index} style={styles.experienceItem}>
                    <View style={styles.jobHeader}>
                      <Text style={styles.jobTitle}>{job.position}</Text>
                      <Text style={styles.dates}>
                        {job.startDate} - {job.endDate || "Present"}
                      </Text>
                    </View>
                    <Text style={styles.jobCompany}>{job.company}</Text>
                    {hasContent(job.highlights) && (
                      <View style={styles.bulletList}>
                        {job.highlights.map((highlight, idx) => (
                          <View key={idx} style={styles.bulletItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.bulletText}>{highlight}</Text>
                          </View>
                        ))}
                      </View>
                    )}
                  </View>
                ))}
              </View>
            )}

            {/* Projects */}
            {hasContent(projects) && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Projects</Text>
                {projects.map((project, index) => (
                  <View key={index} style={styles.experienceItem}>
                    <View style={styles.jobHeader}>
                      <Text style={styles.jobTitle}>{project.name}</Text>
                      {project.startDate && (
                        <Text style={styles.dates}>
                          {project.startDate}{project.endDate ? ` - ${project.endDate}` : ""}
                        </Text>
                      )}
                    </View>
                    {project.description && (
                      <Text style={styles.jobCompany}>{project.description}</Text>
                    )}
                    {hasContent(project.highlights) && (
                      <View style={styles.bulletList}>
                        {project.highlights.map((highlight, idx) => (
                          <View key={idx} style={styles.bulletItem}>
                            <Text style={styles.bullet}>•</Text>
                            <Text style={styles.bulletText}>{highlight}</Text>
                          </View>
                        ))}
                      </View>
                    )}
                  </View>
                ))}
              </View>
            )}

            {/* Education */}
            {hasContent(education) && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Education</Text>
                {education.map((edu, index) => (
                  <View key={index} style={styles.educationItem}>
                    <View style={styles.jobHeader}>
                      <Text style={styles.jobTitle}>
                        {edu.studyType} in {edu.area}
                      </Text>
                      <Text style={styles.dates}>
                        {edu.startDate} - {edu.endDate}
                      </Text>
                    </View>
                    <Text style={styles.jobCompany}>{edu.institution}</Text>
                    {edu.gpa && (
                      <Text style={styles.gpa}>GPA: {edu.gpa}</Text>
                    )}
                  </View>
                ))}
              </View>
            )}

            {/* Awards */}
            {hasContent(awards) && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Awards & Achievements</Text>
                {awards.map((award, index) => (
                  <View key={index} style={styles.awardItem}>
                    <View style={styles.jobHeader}>
                      <Text style={styles.jobTitle}>{award.title}</Text>
                      <Text style={styles.dates}>{award.date}</Text>
                    </View>
                    {award.awarder && (
                      <Text style={styles.awarderText}>{award.awarder}</Text>
                    )}
                    {award.summary && (
                      <Text style={styles.awardSummary}>{award.summary}</Text>
                    )}
                  </View>
                ))}
              </View>
            )}

            {/* Footer */}
            {/* <Text style={styles.footer}>
              Generated with ResumeAI - {new Date().toISOString().split('T')[0]}
            </Text> */}
          </View>
        </Page>
      </Document>
    );
  } catch (error) {
    console.error("Error rendering ModernPDF:", error);
    return (
      <Document>
        <Page size="A4" style={{ padding: 30 }}>
          <View>
            <Text>There was an error generating your PDF.</Text>
            <Text>Please try another template or contact support.</Text>
          </View>
        </Page>
      </Document>
    );
  }
};

// Mark this component as a React-PDF component for proper handling
ModernPDF.isReactPDFComponent = true;

export default ModernPDF;